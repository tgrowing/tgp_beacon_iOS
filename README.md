# 基础背景介绍
<font color="orange"> 接入可直接跳到接入说明 </font>
<font color="orange"> 以下是简单介绍，详细介绍请看**腾讯云DataInsight 开发指引手册** </font>
##### 1. 事件模型（Event Model）
...
在 Params 扩展属性参数这部分中，特殊地，如果使用平台SDK上报，平台会预置一些参数字段作为接口供业务上报。预置字段能够使数据上报更加规范、减少由于对名称理解不一所导致的误解，因此建议尽量使用预置的字段上报对应信息，如果没有相应的预置字段，可以通过定义自定义参数字段来扩展上报。
##### 2.定义事件的 event code 和显示名
现在，产品给到的需要埋点的页面和元素清单。需要开始定义事件了。
...
怎么判断事件是粗了还是细了呢？以[10,500]的合理区间为辅助判断。
（三）事件太少？路径图方法
假如现在定义出来的事件总数太少了 ，少于10条，那么就是事件划分太粗犷了。
试试路径图方法，来重新审视事件划分是否合理，是否应该继续拆分。
具体例子分析请看**腾讯云DataInsight 开发指引手册**
##### 3. 2.3.2  事件日志的埋点开发
